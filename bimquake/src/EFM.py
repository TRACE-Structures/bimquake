...
def create_equivalent_frame_model_from_excel(file_name):
    ...
    floor_props = sheets.get("data", pd.DataFrame())
    wall_props = {}
    #Floor heights
    h = floor_props["H [m]"]
    #Floor weights
...
#--------------------------------------------------------------------------
      ...
      def set_seismic_design_parameters(
          ...
          self.derive_and_set_seismic_loading_params(behaviour_factor)
...
#--------------------------------------------------------------------------
      ...
      def derive_and_set_seismic_loading_params(self, q_ort=3):
          ParaTR = self.ParaTR
          # print(ParaTR)
          ...
          self.derived_design_params = {
            ...
            "q": self.behaviour_factor,  #behavior factor
            "ag_SLV": ag_SLV,
            "S_geo": S_geo,
...
#--------------------------------------------------------------------------
      ...
      def run_linear_static_analysis(self, gamma_m=2):
          ...
          for j, floor in enumerate(self.floors):
            SFs_j = floor.compute_wall_safety_factors(gamma_m)
            SFs.append(SFs_j)
          return SFs
