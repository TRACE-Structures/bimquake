...
import warnings
from .EFM import EquivalentFrameModel
...
################################################################################
    ...
    def generate_excel_from_EFM_props(self, filename):
      wall_props = self.get_wall_partition_properties()
      floor_props = self.get_floor_properties()
      output_file = filename

      with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:

          # Write main data sheet
          floor_props.to_excel(writer, sheet_name="data", index=False)

          # Write each wall_props DataFrame to its own sheet
          for i, df_value in wall_props.items():
              sheet_name = f"Floor{i+1}"
